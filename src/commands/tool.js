...
import { lstat, readFile, rm, rmdir, symlink, writeFile } from "node:fs/promises";
 ...
 */
async function writeAdapterFiles(projectRoot, resolvedRoot, entries) {
  let written = 0;
 ...
 */
async function removeAdapterFiles(projectRoot, resolvedRoot, adapterName, mcpConfig) {
  ...
  for (const entry of entries) {
    ...
    // Use lstat to detect both valid files and dangling symlinks
    try {
...
export async function toolAdd(names) {
  ...
  for (const name of selectedNames) {
    ...
    const entries = adapter.transform(mcpConfig);
    const written = await writeAdapterFiles(projectRoot, resolvedRoot, entries);
...
export async function toolRemove(names) {
  ...
  for (const name of names) {
    const removed = await removeAdapterFiles(projectRoot, resolvedRoot, name, mcpConfig);
